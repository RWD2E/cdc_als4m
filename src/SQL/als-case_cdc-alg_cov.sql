...
/*get definite and probable ALS cohort based on CDC alg
  dependency:   
*/
...
left join NPI_TAXONOMY np on to_char(d.PROVIDERID) = to_char(np.NPI) and np.ENTITY_TYPE_CODE = 1
...
with px_cte as (
    ...
    left join NPI_TAXONOMY np on to_char(p.PROVIDERID) = to_char(np.NPI) and np.ENTITY_TYPE_CODE = 1
...
with ptd_coc_yr as (
    ...
    join ALS_INIT_ENR a on a.patid = e.patid
...
left join NPI_TAXONOMY np on to_char(d.prescriberid) = to_char(np.NPI) and np.ENTITY_TYPE_CODE = 1
...
left join NPI_TAXONOMY no on to_char(e.FACILITYID) = to_char(no.NPI) and no.ENTITY_TYPE_CODE = 2
left join NPI_TAXONOMY np on to_char(e.PROVIDERID) = to_char(np.NPI) and np.ENTITY_TYPE_CODE = 1
...
-- 227	Consultation, evaluation, and preventative care; Other diagnostic procedures (interview, evaluation, consultation)	2,795
-- 231	Other therapeutic procedures	2,657
-- 233	Laboratory - Chemistry and Hematology	2,343
-- 235	Other Laboratory	2,261
-- 999	HCPCS Level II codes not classified;
...
------------------------------------------------------------------------------
/* Neurology care before and after */
create or replace table NEUROLOGIST_CARE as
with neuro_bef_stk as (
    -- select patid, min(DAYS_SINCE_ALS1DX) as days_since_als1dx
    -- from ALL_DIAGNOSIS_BEFORE_ALS1DX where NEUROLOGY_FLAG = 1
    -- group by patid
    -- union
    select patid, min(DAYS_SINCE_ALS1DX) as days_since_als1dx
    from ALL_PROCEDURES_BEFORE_ALS1DX where NEUROLOGY_FLAG = 1
    group by patid
    -- union
    -- select patid, min(DAYS_SINCE_ALS1DX) as days_since_als1dx
    -- from ALL_DISPENSING_BEFORE_ALS1DX where NEUROLOGY_FLAG = 1
    -- group by patid
    -- union
    -- select patid, min(DAYS_SINCE_ALS1DX) as days_since_als1dx
    -- from ALL_ENCOUNTER_BEFORE_ALS1DX where NEUROLOGY_FLAG = 1
    -- group by patid
),  neuro_aft_stk as (
    -- select patid, min(DAYS_SINCE_ALS1DX) as days_since_als1dx
    -- from ALL_DIAGNOSIS_AFTER_ALS1DX where NEUROLOGY_FLAG = 1
    -- group by patid
    -- union
    select patid, min(DAYS_SINCE_ALS1DX) as days_since_als1dx
    from ALL_PROCEDURES_AFTER_ALS1DX where NEUROLOGY_FLAG = 1
    group by patid
    -- union
    -- select patid, min(DAYS_SINCE_ALS1DX) as days_since_als1dx
    -- from ALL_DISPENSING_AFTER_ALS1DX where NEUROLOGY_FLAG = 1
    -- group by patid
    -- union
    -- select patid, min(DAYS_SINCE_ALS1DX) as days_since_als1dx
    -- from ALL_ENCOUNTER_AFTER_ALS1DX where NEUROLOGY_FLAG = 1
    -- group by patid
)
select a.patid,
       min(b.days_since_als1dx) as neurologist_bef_als1dx,
       min(c.days_since_als1dx) as neurologist_aft_als1dx
from ALS_INIT_ENR a
left join neuro_bef_stk b on a.patid = b.patid
left join neuro_aft_stk c on a.patid = c.patid
group by a.patid
;

select * from NEUROLOGIST_CARE;
